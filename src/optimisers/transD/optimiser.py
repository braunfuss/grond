...
class GuidedSamplerPhase(SamplerPhase):
    ...
    def get_raw_sample(self, problem, iiter, chains, misfits):
        ...
        #                sampled = 'grad'
            #            sampled_strike = self.prior_grad_strike.rvs()
            #            if sampled_strike >= xbounds[6+12*i, 0] and\
            #               sampled_strike <= xbounds[6+12*i, 1]:
            #                check_bounds_lf = False
            #                model[6+12*i] = sampled_strike
                if self.grad_input_grid is not None and\
                   self.bp_input_grid_lf is not None:
                    sampled_index_xy = []
                    weight_grad = 5
                    #sampled.append('LF')
                    for l in range(0, weight_grad):
                        sampled.append('grad')
                    sampled = rstate.choice(sampled, 1)[0]
                    if sampled is 'LF':
                        sampled_index_xy = self.prior_bp_loc.rvs()
                sampled_index_xy = self.prior_grad_loc.rvs()
                sampled_index_xy = sampled_index_xy
